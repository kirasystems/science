...
from utils import HParams, preprocess, iter_data, concatenate_for_encoder, load_sst
import pprint
...
class Model(object):
    ...
    def __init__(self, restore_path, nbatch=128, nsteps=64):
        ...
        def transform(xs):
            ...
            maxlen = float(np.max(lens)) # THIS MUST BE A FLOAT
            offset = 0
            n = len(xs)
            ...
            for step in range(0, ceil_round_step(maxlen, nsteps), nsteps):
                ...
                for batch in range(0, nsubseq, nbatch):
                    ...
                    smb[:, offset+start:offset+end, :] = batch_smb # end states overwrite old ones for unfinished sequences
